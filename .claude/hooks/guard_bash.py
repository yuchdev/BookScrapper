...
``BOOK_SCRAPPER_PROD_CONFIRMED=1`` in the environment before launching Claude Code.
...
def main() -> None:
    ...
    for pattern, reason in DESTRUCTIVE:
        if re.search(pattern, command, re.IGNORECASE):
            ...
            block(
                f"Blocked by Book Scrapper bash-guard: {reason}.\n"
                f"Command: {command}\n"
                "This operation is destructive and is never allowed automatically. "
                "If you are certain, ask the user to run it manually."
            )

    if PROD.search(command) and os.environ.get("BOOK_SCRAPPER_PROD_CONFIRMED") != "1":
        append_log("bash-guard.log", f"BLOCKED [prod-target] :: {command}")
        block(
            "Blocked by Book Scrapper bash-guard: command targets a production resource.\n"
            f"Command: {command}\n"
            "Set BOOK_SCRAPPER_PROD_CONFIRMED=1 in the environment to authorise production "
