...
1. Establish current coverage:
   `uv run pytest tests/unit/ -q --cov=book_scrapper --cov-report=term-missing`
   (scope with `--cov=book_scrapper.<subpkg>` when a path is given).
2. Spawn the **`testing-expert`** agent with the coverage output and the target path. Ask it
   ...
   (blast-radius criteria + Book Scrapper module examples per tier):
